<section class="footer-all">
	...
	<div class="flex justify-center w-1/2 m-auto">
		<a href="{{ route('full-story', ['locale' => app()->getLocale()]) }}" class="btn-couture">{{ __('footer.all-story') }}</a>
		<a href="#" class="btn-couture">{{ __('footer.all-chart') }}</a>
		<a href="{{ route('client-service', ['locale' => app()->getLocale(), 'page' => 'faq']) }}" class="btn-couture">{{ __('footer.all-faq') }}</a>
	</div>
</section>